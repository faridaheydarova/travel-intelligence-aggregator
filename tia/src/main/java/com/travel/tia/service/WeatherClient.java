...
public class WeatherClient {
    ...
    public Mono<WeatherNow> getWeather(double lat, double lon) {
        WeatherClient.log.info("Weather API call: lat={}, lon={}", lat, lon);
        ...
        response-> {
            log.error("Weather API returned HTTP error: {}", response.statusCode());
          return  Mono.error(new WeatherServiceException("Weather API ERROR"));
        }
                )
